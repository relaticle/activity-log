...
**Renderer.** A class implementing `Relaticle\ActivityLog\Contracts\TimelineRenderer`. Given a `TimelineEntry`, it returns a `View` or `HtmlString`. Renderers are looked up by the entry's `renderer` field (explicit) or its `type` (fallback) via the `RendererRegistry`. See [/essentials/renderers](/essentials/renderers).
...
Critically: RelatedActivityLogSource emits entries with `type='activity_log'` (**not** `'related_activity_log'`). Filtering with `->ofType(['related_activity_log'])` will never match anything. To target related-model activity entries specifically, filter on the `relatedModel` field or on the related model's class — not on `type`.
