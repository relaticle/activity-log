...
> Compose sources — own log, related logs, timestamp columns, custom closures.
...
<tbody>
  <tr>
    ...
    <td>
      Register one event per timestamp column. <code>
       ...
       is an optional row-level filter (return <code>
        bool
      </code>
  ...
  <tr>
    ...
    <td>
      Resolves the actor. <code>
        string
      </code>
      
       is a relation name on the row; closure returns a <code>
        Model
      </code>
